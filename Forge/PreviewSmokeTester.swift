...
final class PreviewSmokeTester: NSObject, WKScriptMessageHandler, WKNavigationDelegate {
    ...
    func run(_ url: URL, timeout: Duration = .seconds(15)) async -> [RuntimeIssue] {
        ...
        controller.add(self, name: "forge")
    ...
    private func finish() {
        ...
        window?.orderOut(nil)
        window = nil
        webView?.configuration.userContentController.removeScriptMessageHandler(forName: "forge")
        webView = nil
    ...
    (function () {
      ...
      async function run(){
        ...
        try { window.webkit.messageHandlers.forge.postMessage({ kind: 'smokeDone' }); } catch (e) {}
      }
