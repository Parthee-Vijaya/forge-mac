...
final class HostClient {
    ...
    var host: String = UserDefaults.standard.string(forKey: "storm-host") ?? ""
    var connection: Connection = .idle
    var token: String = UserDefaults.standard.string(forKey: "storm-token") ?? ""   // Fase 4c: steer token
    ...
    func connect() {
        UserDefaults.standard.set(host, forKey: "storm-host")
        task?.cancel()
    ...
    private func friendly(_ error: Error) -> String {
        ...
        case NSURLErrorCannotConnectToHost, NSURLErrorTimedOut, NSURLErrorCannotFindHost,
            ...
            return "Kan ikke nå Mac'en. Kører Stormbreaker, og er du på samme netværk/Tailscale?"
        default:
    ...
    func steer(_ path: String, body: [String: Any] = [:]) {
        UserDefaults.standard.set(token, forKey: "storm-token")
        guard !hostOnly.isEmpty, let url = URL(string: "http://\(hostOnly):7842/\(path)") else { return }
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.timeoutInterval = 6
        req.setValue(token, forHTTPHeaderField: "X-Stormbreaker-Token")
