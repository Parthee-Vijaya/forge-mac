...
/// The Stormbreaker anvil mark — the same silhouette as the app icon, as a vector Shape
...
struct StormbreakerBadge: View {
    ...
    var body: some View {
        RoundedRectangle(cornerRadius: size * 0.2265, style: .continuous)
            ...
            .overlay(StormbreakerMark().fill(Theme.accent).frame(width: size * 0.66, height: size * 0.66))
