...
struct OnboardingView: View {
    ...
    private var header: some View {
        HStack(spacing: 9) {
            StormbreakerBadge(size: 18)
            Text("Stormbreaker").font(Theme.wordmark(16)).foregroundStyle(Theme.ink)
            Spacer()
    ...
    private func content(_ model: Bindable<AppModel>) -> some View {
        ...
        case 3:
            stepShell("Kør Stormbreaker lokalt — gratis og privat",
                      "Stormbreaker bruger en AI-model til at skrive koden. En lokal model kører på din egen Mac — gratis, offline og privat. Jeg har fundet den bedste til din hardware.") {
        ...
        case 5:
            stepShell("GitHub (valgfri)",
                      "Stormbreaker kan gemme din kode på GitHub — så du har en sikkerhedskopi i skyen og kan deploye. Log ind med `gh auth login` i Terminal; så finder Stormbreaker automatisk dit brugernavn herunder. Kan springes over og sættes op senere.") {
                VStack(alignment: .leading, spacing: 10) {
        ...
        case 7:
            stepShell("Global memory (valgfri)",
                      "Ting Stormbreaker altid skal huske om dig og dine præferencer — det føjes til hver eneste build-tur, så du ikke skal gentage det. Fx: “TypeScript strict, minimale dependencies, dansk UI-tekst, undgå inline styles”.") {
    ...
    private func welcomeStep(_ model: Bindable<AppModel>) -> some View {
        VStack(alignment: .leading, spacing: 22) {
            HStack(spacing: 14) {
                StormbreakerBadge(size: 56)
                VStack(alignment: .leading, spacing: 3) {
                    Text("Velkommen til Stormbreaker")
            ...
            Text("Stormbreaker er en lokal app-bygger: du skriver hvad du vil have, en AI skriver koden, og du ser den køre live med det samme. Du behøver ikke kunne kode. Lad os sætte dig op — det tager cirka et minut.")
            ...
            Toggle(isOn: model.preferences.learningMode) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("Slå learning mode til: Stormbreaker forklarer hvad der sker undervejs, har en ordbog over fagudtryk, og guider dig gennem build, fejl, kode og deployment.")
                        .font(.system(size: 12.5)).foregroundStyle(Theme.inkSoft)
                        .fixedSize(horizontal: false, vertical: true)
                }
            }
