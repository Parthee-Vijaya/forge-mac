    ...
    var body: some View {
        ...
        Form {
            Section("Dig") {
                ...
                Text("Sådan hilser Stormbreaker på dig på startskærmen — “Hvad vil du bygge, P?”.")
            ...
            Section("Adfærd") {
                ...
                Text("Når slået til retter Stormbreaker automatisk en fejl i den kørende app uden du beder om det. Kan altid rulles tilbage via checkpoints. ‘Fix it’-knappen er tilgængelig uanset.")
                    .font(.system(size: 11)).foregroundStyle(.secondary)
                Toggle("Funktionel røgtest af preview", isOn: $model.preferences.functionalSmokeTest)
                Text("Når en build ellers er ren, afprøver Stormbreaker automatisk den kørende app (skriver i felter, klikker på knapper) i en skjult fane for at fange fejl der først opstår ved interaktion. Tilføjer et par sekunder ved den rene build.")
                    .font(.system(size: 11)).foregroundStyle(.secondary)
                ...
                Text("Stormbreaker beder om lov før den kører disse handlinger under et build. Fil-ændringer spørges der aldrig om — de kan altid fortrydes via checkpoints.")
                    .font(.system(size: 11)).foregroundStyle(.secondary)
