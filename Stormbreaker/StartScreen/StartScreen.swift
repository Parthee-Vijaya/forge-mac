...
struct StartScreen: View {
    ...
    private func sidebar(_ model: AppModel) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 9) {
                StormbreakerBadge(size: 22)
                Text("Stormbreaker").font(Theme.wordmark(18)).foregroundStyle(Theme.ink)
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("Bruges når Stormbreaker hilser på dig — fx “Hvad vil du bygge, P?”. Du kan altid ændre det i Indstillinger.")
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            VStack(alignment: .leading, spacing: 6) {
                Text("Klon fra Git")
                    .font(.system(size: 20, weight: .semibold)).foregroundStyle(Theme.ink)
                Text("Indsæt en repo-URL. Stormbreaker kloner den til et nyt projekt og starter den, hvis det er et Node/Vite-projekt.")
                    .font(.system(size: 13)).foregroundStyle(Theme.inkSoft)
                    .fixedSize(horizontal: false, vertical: true)
            }
