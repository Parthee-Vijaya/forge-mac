...
/// Root app state and the glue between the SwiftUI UI and the StormbreakerKit engine.
@MainActor
@Observable
final class AppModel: PermissionGate {
    ...
    // StormbreakerKit handles (Sendable; safe to use from off-main tasks).
    // Config
    ...
    @ObservationIgnored private(set) var processLayer: StormbreakerProcessLayer
    ...
    init() {
        ...
        self.processLayer = StormbreakerProcessLayer(workspace: workspace, devServer: devServer)
    ...
    func refreshModels() async {
        ...
        } else if let key = ProcessInfo.processInfo.environment["STORM_CLOUD_API_KEY"], !key.isEmpty {
            let cloud = ProcessInfo.processInfo.environment["STORM_CLOUD_MODEL"] ?? "nvidia/llama-3.1-nemotron-70b-instruct"
    ...
    private func composedSystemPrompt(role: ModelRole, config: ModelConfig) async -> String {
        ...
        case .build: base = SystemPrompt.storm(lineReplace: config.supportsLineReplace)
    ...
    func deploy() {
        ...
        deployGithubURL = nil
        deployLiveURL = nil
        let repo = "storm-" + Self.slug(currentProject.name)
        Task {
    ...
    private func runDeploy(repo: String) async {
        ...
        _ = try? await deployShell(
            ...
            + "git commit -q -m 'Deploy from Stormbreaker' 2>/dev/null || true")
    ...
    func exportZip() {
        ...
        panel.nameFieldStringValue = "storm-\(Self.slug(currentProject.name)).zip"
    ...
    func exportChatMarkdown() {
        ...
        panel.nameFieldStringValue = "storm-\(Self.slug(currentProject.name)).md"
        panel.canCreateDirectories = true
        guard panel.runModal() == .OK, let out = panel.url else { return }
        do {
    ...
    func chatMarkdown() -> String {
        ...
        lines.append("---")
        var footer = "Bygget med Stormbreaker · \(Self.formatTokens(sessionTokens)) tokens"
    ...
    /// B20: export the project as a shareable `.storm` bundle — source + chat
    ...
    /// B20: import a `.storm` bundle as a new project — unzip, read the manifest +
    /// any chat history, then npm install + start (node_modules isn't bundled).
    func importBundle() {
        guard !isBusy else { return }
        let panel = NSOpenPanel()
        panel.title = "Importér Stormbreaker-bundle"
        ...
        Task {
            ...
            messages = !importedChat.isEmpty ? importedChat : [UIMessage(role: .assistant, text:
                ...
                : "Kunne ikke pakke bundlen ud — er filen en gyldig .storm-bundle?")]
            await refreshFiles()
            statusText = "Ready."
            isBusy = false
            persistCurrentChat()
    ...
    func cloneFromGit() {
        ...
        cloneTask = Task {
            ...
            if cloned, await workspace.fileExists("package.json") {
                ...
                templateInstalled = true
                let server = devServer
                Task { try? await server.start() }   // fire-and-stormt; preview fills in via the log stream
    ...
    private func beginTurn(visiblePrompt: String, modelPrompt: String,
        ...
        agentTask = Task {
            ...
            if !cancelled {
                ...
                // Native notification when Stormbreaker is in the background.
                if role == .build {
                    if case .clean = phase { notifyIfBackgrounded("Stormbreaker", "Build færdig ✓") }
    ...
    func explainTerm(_ term: String) async -> String {
        ...
        let user = "Forklar begrebet \"\(term)\", som det bruges i en vibecoding-app som Stormbreaker."
        do {
            let provider = ModelRouter.provider(for: config)
            let options = ModelRouter.options(for: config)
            var out = ""
    ...
    private func runAgent(prompt: String, history: [ChatMessage], assistantIndex: Int,
        ...
        for await event in StormbreakerEngineFactory.make(.storm, deps: deps)
    ...
    /// Stormbreaker isn't the active app — in the foreground the in-app toast suffices.
    ...
    /// recent-projects list can show a visual thumbnail. Fire-and-stormt; reuses
    ...
    /// Fase 3b: project files the user pinned via @file (delegates to StormbreakerKit).
    ...
    var displayStatus: String {
        ...
        case .startingServer, .running: return "Stormbreaker starter preview…"
        case .stopped: return "Preview-serveren stoppede — genstart for at se den."
        case .idle:
            switch phase {
            case .clean, .idle, .planReady: return "Stormbreaker starter preview…"
    ...
    func restartDevServer() {
        ...
        statusText = "Stormbreaker starter preview…"
        let server = devServer
        Task { try? await server.restartForDependencyChange() }
    }
