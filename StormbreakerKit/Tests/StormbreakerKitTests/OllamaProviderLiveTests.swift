...
/// STORM_RUN_INTEGRATION=1 (requires Ollama running with the model pulled).
final class OllamaProviderLiveTests: XCTestCase {
    func testStreamsTokensFromLocalModel() async throws {
        try XCTSkipUnless(
            ...
            "set STORM_RUN_INTEGRATION=1 (and run Ollama) to run the live model test"
        )
        ...
        let messages = [
            ...
            ChatMessage(role: .user, content: "Say hello to Stormbreaker."),
        ]
