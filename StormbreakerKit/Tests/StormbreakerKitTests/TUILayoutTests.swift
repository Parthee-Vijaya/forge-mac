...
final class TUILayoutTests: XCTestCase {
    ...
    func testStormbreakerLayoutStandard() {
        let l = StormbreakerLayout.compute(Size(cols: 80, rows: 24))   // header / body / input(3) / status(1)
    ...
    func testStormbreakerLayoutNarrowCollapsesSide() {
        let l = StormbreakerLayout.compute(Size(cols: 50, rows: 24))
    ...
    func testSlashAnchorSitsAboveInput() {
        let l = StormbreakerLayout.compute(Size(cols: 80, rows: 24))
