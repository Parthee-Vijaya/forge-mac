...
final class LineReplaceTests: XCTestCase {
    ...
    func testSystemPromptIncludesLineReplaceOnlyForStrongModels() {
        XCTAssertFalse(SystemPrompt.storm(lineReplace: false).contains("line-replace"))
        XCTAssertTrue(SystemPrompt.storm(lineReplace: true).contains("SEARCH"))
    }

    func testFewShotExampleMatchesCapability() {
        // Weak models get a concrete whole-file example…
        let weak = SystemPrompt.storm(lineReplace: false)
        ...
        let strong = SystemPrompt.storm(lineReplace: true)
