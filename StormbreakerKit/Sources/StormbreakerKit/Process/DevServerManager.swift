...
/// dependency install → dev-server launch (via the storm-run.sh wrapper) →
...
public actor DevServerManager {
    ...
    private func launchDevServer(
        ...
        // Stormbreaker parses the actual port from stdout.
        ...
        // via 127.0.0.1; Stormbreaker parses the actual Local port from stdout.
        let portArgs = ["--", "--port", "\(projectPort)", "--host"]
        var arguments = packageManager.devArgs + portArgs

        // Prefer the storm-run.sh wrapper for orphan safety; fall back to a
        // direct launch if it can't be written.
        if let wrapper = try? materializeRunWrapper() {
            devEnv["STORM_PARENT_PID"] = "\(ProcessInfo.processInfo.processIdentifier)"
            executableURL = URL(fileURLWithPath: "/bin/sh")
    ...
    /// A stable, per-project dev-server port in a Stormbreaker range (5300–5699), well
    ...
    private func materializeRunWrapper() throws -> URL {
        ...
        let url = dir.appendingPathComponent("storm-run.sh")
