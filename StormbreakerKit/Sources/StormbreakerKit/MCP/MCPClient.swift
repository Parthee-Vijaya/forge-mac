...
/// and speaks newline-delimited JSON-RPC 2.0 to it. Lets Stormbreaker's agent USE external
/// tools (filesystem, fetch, git, … — or storm-mcp itself). Sequential request/
...
public final class MCPClient: @unchecked Sendable {
    ...
    public init(server: String, command: String, args: [String], env: [String: String], cwd: URL) {
        self.server = server
        self.queue = DispatchQueue(label: "storm.mcp.\(server)")
        let inPipe = Pipe(), outPipe = Pipe()
        inWrite = inPipe.fileHandleForWriting
        outRead = outPipe.fileHandleForReading
        // Login shell so npx/uvx/etc. resolve on PATH even when Stormbreaker is a GUI app.
    ...
    public func start() async throws {
        ...
        _ = try await rpc("initialize", [
            ...
            "clientInfo": ["name": "storm", "version": "0.1.0"],
        ])
