...
// ─────────────────────────────────────────────────────────────────────────────
// `storm` — the Stormbreaker engine, on the command line.
//
...
//
//   storm new <name> [--framework react|svelte|vue|nextjs]
//   storm build "<prompt>" [--project DIR] [--framework F] [--model M]
//                          [--provider lmStudio|ollama|openai|anthropic|gemini|nvidia]
//                          [--plan] [--plain]
//   storm chat [--project DIR] [--framework F] ...    # interactive REPL
//
...
func fail(_ s: String) -> Never { FileHandle.standardError.write(Data((red("storm: ") + s + "\n").utf8)); exit(1) }

// MARK: - Config

struct StormbreakerConfig: Codable {
    var provider: String?
    var model: String?
    ...
    static func load() -> StormbreakerConfig {
        let path = configDir().appendingPathComponent("config.json")
        guard let data = try? Data(contentsOf: path),
              let cfg = try? JSONDecoder().decode(StormbreakerConfig.self, from: data)
        else { return StormbreakerConfig() }
        return cfg
    }
...
func makeModelConfig(_ args: Args, _ cfg: StormbreakerConfig) -> ModelConfig {
...
func makeDeps(_ engine: Engine, mode: AgentLoop.Mode, gate: (any PermissionGate)? = nil,
    ...
    let processLayer = StormbreakerProcessLayer(workspace: engine.workspace, devServer: engine.devServer)
    // Strong models (cloud) get the line-replace prompt; local models keep full-file writes.
    let base = mode == .plan ? SystemPrompt.plan : SystemPrompt.storm(lineReplace: engine.config.supportsLineReplace)
...
/// Session-wide accumulator across every model call in this `storm` run.
...
final class StdinPermissionGate: PermissionGate, @unchecked Sendable {
    ...
    func decide(_ request: PermissionRequest) async -> PermissionDecision {
        ...
        say("\n" + cyan("⚠ Stormbreaker vil \(request.label)"))
...
func slug(_ s: String) -> String {
    ...
    return joined.isEmpty ? "storm-app" : joined
}
...
\(bold("KOMMANDOER"))
  ...
  storm skills [--project DIR]          Vis tilgængelige skills (presets)
  storm mcp [--project DIR]             Start MCP-servere fra .forge/.mcp.json + vis værktøjer

\(bold("VALG"))
  --project DIR        Projektmappe (default: ./<navn> eller ./<slug>)
  --framework F        react (default) · svelte · vue · nextjs
  --skill <id>         brug en skill som prompt (se 'storm skills'); positional = ekstra input
  ...
  --api-key KEY        cloud-nøgle (ellers $STORM_CLOUD_API_KEY eller config)
  ...
  --resume             genoptag seneste session (storm chat i TUI)
...
func cmdNew(_ args: Args, _ cfg: StormbreakerConfig) async {
    guard let name = args.positionals.first else { fail("brug: storm new <navn> [--framework F]") }
    ...
    do {
        ...
        info("kør:  storm chat --project \(name)")
    } catch { fail("kunne ikke scaffolde: \(error)") }
}

func cmdBuild(_ args: Args, _ cfg: StormbreakerConfig) async {
    let framework = Framework(id: args.option("framework") ?? cfg.framework ?? "react")
    ...
    if let skillID = args.option("skill") {
        ...
        guard let skill = SkillStore.find(skillID, in: skills) else {
            fail("ukendt skill '\(skillID)'. Se: storm skills")
        }
        ...
        guard let p = args.positionals.first, !p.isEmpty else {
            fail("brug: storm build \"<prompt>\"   ·   eller: storm build --skill <id>")
        }
        prompt = p
...
func cmdChat(_ args: Args, _ cfg: StormbreakerConfig) async {
    let framework = Framework(id: args.option("framework") ?? cfg.framework ?? "react")
    let dir = resolveProjectDir(args, defaultName: "storm-app")
    let config = makeModelConfig(args, cfg)
    info("model: \(config.displayName) (\(config.source.rawValue)) · framework: \(framework.displayName)")
    var engine: Engine
...
func cmdSkills(_ args: Args, _ cfg: StormbreakerConfig) {
    ...
    say(dim("\nBrug:  storm build --skill <id> [\"ekstra tekst\"]   ·   i chat:  :skill <id> [tekst]"))
}

func cmdMCP(_ args: Args, _ cfg: StormbreakerConfig) async {
    let dir = args.option("project").map { URL(fileURLWithPath: ($0 as NSString).expandingTildeInPath) }
...
/// `storm __tuicheck` — phase-1 lifecycle probe: enter raw mode + alt screen,
...
func cmdTUICheck() {
    ...
    do { try term.enter() } catch { fail("\(error)") }
    defer { term.restore() }
    let msg = "Stormbreaker TUI ✓   \(term.cols)×\(term.rows)   — tryk q for at afslutte"
