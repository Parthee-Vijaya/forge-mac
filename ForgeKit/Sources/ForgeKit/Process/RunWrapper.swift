...
/// The dev-server wrapper script, materialized to `<project>/.forge/forge-run.sh`
...
/// reaps the subtree if the Forge parent process dies (Foundation `Process`
...
enum RunWrapper {
    ...
    WATCHDOG=""
    if [ -n "${FORGE_PARENT_PID:-}" ]; then
      (
        while kill -0 "$FORGE_PARENT_PID" 2>/dev/null; do
          sleep 1
        done
        cleanup
        sleep 2
