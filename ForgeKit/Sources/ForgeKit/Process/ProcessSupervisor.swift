...
public struct ProcessSupervisor: Sendable {
    ...
    /// previous Forge session, across ALL projects under `projectsRoot`. Meant
    /// for app launch, when no Forge dev server is running yet — so every match
    ...
    /// references the Forge projects directory AND looks like a dev server (a
    /// `vite` process or the `forge-run.sh` wrapper) — an editor that merely has
    ...
    /// Forge projects path) plus a dev-server marker. `ps -axww` gives full argv.
    private static func devServerPIDs(referencing needle: String) -> [Int32] {
        ...
        for raw in output.split(separator: "\n") {
            ...
            guard lower.contains("vite") || lower.contains("forge-run.sh") else { continue }
            pids.append(pid)
        }
        return pids
    }
