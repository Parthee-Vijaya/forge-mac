...
/// FORGE_RUN_INTEGRATION=1 (needs Ollama running with qwen2.5-coder:14b).
final class FullLoopLiveTests: XCTestCase {
    func testEndToEndBuildsAndServesGeneratedApp() async throws {
        try XCTSkipUnless(
            ...
            "set FORGE_RUN_INTEGRATION=1 (and run Ollama) to run the full end-to-end test"
        )

        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("forge-e2e-\(UUID().uuidString)")
        ...
        let process = ForgeProcessLayer(workspace: workspace, devServer: devServer)
        ...
        let prompt = """
        Replace src/App.tsx with a centered page: a big heading that says "Hello Forge" and a \
