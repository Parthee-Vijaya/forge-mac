...
/// `npm install`), so gated behind FORGE_RUN_INTEGRATION=1:
///
///   FORGE_RUN_INTEGRATION=1 swift test --filter DevServerIntegrationTests
final class DevServerIntegrationTests: XCTestCase {
    func testStartsServesAndStopsDevServer() async throws {
        try XCTSkipUnless(
            ...
            "set FORGE_RUN_INTEGRATION=1 to run the slow npm/vite integration test"
        )
